...
#IDA from ida.obspy.core.util import NATIVE_BYTEORDER
from ida.obspy.core.util.base import NATIVE_BYTEORDER
from ida.obspy.core.util.deprecation_helpers import (
...
def _read_mseed(mseed_object, starttime=None, endtime=None, headonly=False,
    ...
    #IDA return NONE if no contents found
    except ValueError:
    ...
    while True:
        ...
        while True:
            ...
            #IDA trace = Trace(header=header, data=data)
            # Append information.
            #IDA for key, value in info.items():
            #IDA     setattr(trace.stats.mseed, key, value)
            # header['mseed'] = {}
            for key, value in info.items():
                #IDA setattr(header['mseed'], key, value)
    ...
    del lil  # NOQA
    #IDA return Stream(traces=traces)
    return traces
